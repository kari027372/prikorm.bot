...
function handleDocumentClick(event) {
    ...
    if (backBtn) {
        event.preventDefault();
        if (window.ui && window.ui.previousScreen) {
            window.render(window.ui.previousScreen);
        } else {
            window.render('home');
        }
        return;
    ...
    if (themeBtn) {
        ...
        if (typeof window.toggleTheme === 'function') {
            ...
            console.warn('⚠️ toggleTheme не определена');
        }
        return;
    }
    ...
    // --- ДАЛЕЕ ИДЁТ СТАРАЯ ЛОГИКА (без изменений) ---
...
function handleDocumentChange(event) {
    ...
    if (target.id === "food-new-product") {
        return;
    }

    if (target.id === "food-preparation") {
        return;
    }

    if (target.id === "food-form") {
        return;
    }
}
...
// ===== ИЗМЕНЕНО: поддержка новой архитектуры =====
...
function changeFoodSource(source) {
    ...
    if (storeFields) {
        storeFields.hidden = CURRENT_FOOD_SOURCE !== "store";
    }

    if (homemadeFields) {
        homemadeFields.hidden = CURRENT_FOOD_SOURCE !== "homemade";
    }
}
...
function openDiaryEditModal(entryId) {
    var entry = STATE.diary.find(function(item) {
...
function changeDay(direction) {
    ...
    if (typeof renderDailyPlan === "function") {
        renderDailyPlan(formatDate(CURRENT_DATE));
    }
...
function updateTodayDate() {
    ...
    element.textContent = CURRENT_DATE.toLocaleDateString("ru-RU", {
        day: "numeric",
        month: "long"
    });
}
...
function openBabyEditModal() {
    ...
    root.innerHTML =
        ...
        "Имя малыша" +
        '<input id="baby-name-input" value="' + escapeHTML(baby.name || "") + '" placeholder="Имя">' +
        "</label>" +

        '<label class="form-label">' +
        "Дата рождения" +
        '<input id="baby-birth-input" type="date" value="' + escapeHTML(baby.birthDate || "") + '">' +
        "</label>" +
        ...
        '<option value="ГВ"' + (baby.feedingType === "ГВ" ? " selected" : "") + ">" +
        "Грудное вскармливание" +
        "</option>" +

        '<option value="ИВ"' + (baby.feedingType === "ИВ" ? " selected" : "") + ">" +
        "Искусственное" +
        "</option>" +

        '<option value="Смешанное"' + (baby.feedingType === "Смешанное" ? " selected" : "") + ">" +
        "Смешанное" +
        "</option>" +
