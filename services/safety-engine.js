...
(function() {
  ...
  function evaluateProductSafety(profile, product, servingForm) {
    ...
    if (
      ...
      Array.isArray(product.ageRestrictions)
    ) {
      for (const restriction of product.ageRestrictions) {
        if (
          restriction.type === 'absolute' &&
          childAge < restriction.untilMonths
        ) {
          statuses.push('block');

          reasons.push(
            restriction.reason || 'Возрастное ограничение'
          );

          details.age = {
            status: 'blocked_by_age',
            untilMonths: restriction.untilMonths,
    ...
    if (product.mercuryRisk === 'avoid') {
      ...
      reasons.push(
        'Высокое содержание ртути — избегать'
      );
    ...
    if (
      product.rules?.age?.minMonths !== undefined &&
      product.rules?.age?.minMonths !== null
    ) {
      minAge = product.rules.age.minMonths;
    } else if (
      product.introduction?.fromMonths !== undefined &&
      product.introduction?.fromMonths !== null
    ) {
    ...
    if (childAge !== null && minAge !== null) {
      if (childAge < minAge) {
        // Если ещё нет блока от ageRestrictions,
        // добавляем block
        ...
        reasons.push(
          `Возраст ${Math.round(childAge)} мес. меньше рекомендуемого минимума (${minAge} мес.)`
        );

        details.age = {
          status: 'too_early',
          minMonths: minAge,
          actualMonths: childAge
        };
      } else {
        details.age = {
          status: 'appropriate',
      ...
      if (minAge !== null) {
        // Если возраст не определён, но есть minAge –
        // добавляем review
        statuses.push('review');

        reasons.push(
          'Возраст ребёнка не определён, рекомендуется уточнить дату рождения'
        );
    ...
    // ---------- 5. Аллергия ----------
    // Информационная, блок только при совпадении
    ...
    if (product.rules?.allergy) {
      isAllergen =
        product.rules.allergy.isAllergen === true;

      allergenTypes =
        product.rules.allergy.types || [];
    } else {
      ...
      if (Array.isArray(product.allergenType)) {
        ...
        // Используем getProductAllergens из safety.js,
        // если доступен
        const allergensFromOld =
          window.getProductAllergens
            ? window.getProductAllergens(product)
            : [];
    ...
    if (
      ...
      const hasMatchingAllergy =
        childAllergies.some(allergy =>
          allergenTypes.some(type =>
            allergy
              .toLowerCase()
      ...
      if (hasMatchingAllergy) {
        ...
        reasons.push(
          `У ребёнка отмечена аллергия на этот продукт (${allergenTypes.join(', ')})`
        );
    ...
    if (product.rules?.serving) {
      allowedForms =
        product.rules.serving.allowed || [];

      blockedForms =
        product.rules.serving.blocked || [];
    ...
    if (servingForm) {
      if (usingRules) {
        ...
        if (isBlocked) {
          ...
          reasons.push(
            `Форма подачи "${servingForm}" явно не рекомендуется для этого продукта`
          );
          ...
          // Форма не указана ни в allowed,
          // ни в blocked – fallback
          const safeForms =
            product.safeForms || [];
          ...
          if (isUnsafe) {
            ...
            reasons.push(
              `Форма подачи "${servingForm}" небезопасна для этого продукта`
            );
            ...
            details.serving = {
              allowed: null,
              form: servingForm,
              note: 'Форма не указана в правилах'
            };
          }
        }
      } else {
        // Нет rules.serving –
        // используем safeForms/unsafeForms напрямую
        const safeForms =
          product.safeForms || [];
    ...
    if (product.rules?.safety?.chokingRisk) {
      chokingRisk =
        product.rules.safety.chokingRisk;
    ...
    // Если chokingRisk высокий/средний,
    // и нет уже блока от формы, добавляем caution
    if (
      chokingRisk === 'high' ||
      chokingRisk === 'medium'
    ) {
      ...
      if (chokingRisk === 'high') {
        reasons.push(
          'Продукт имеет высокий риск удушья, необходима правильная форма подачи и наблюдение'
        );
        ...
        reasons.push(
          'Продукт требует внимания при подаче'
        );
    ...
    if (medicalNote) {
      reasons.push(
        `Медицинское замечание: ${medicalNote}`
      );
    }

    // ---------- 9. labelChecks ----------
    // Добавленный сахар / соль
    if (
      product.labelChecks &&
      ...
      if (
        product.labelChecks.indexOf('addedSugar') !== -1
      ) {
        warnings.push(
      ...
      if (
        product.labelChecks.indexOf('addedSalt') !== -1
      ) {
        warnings.push(
          'Содержит добавленную соль'
        );
      }
    ...
    // {
    //   [productId]: {
    //     status: 'introduced' | 'suspectedReaction' | ...
    //   }
    // }
    //
    // Поэтому здесь НЕ используем Array.isArray()
    // и НЕ делаем .find().
    //
    // Проверяем ТОЛЬКО текущий product.id.
    // Никаких категорий и глобальных блокировок.

    const productStateMap =
      (
        profile.productState &&
        typeof profile.productState === 'object'
      )
        ? profile.productState
        : {};

    const productStateRecord =
      productStateMap[product.id] || null;

    if (productStateRecord) {
      if (
        productStateRecord.status ===
        'confirmedAllergy'
      ) {
        statuses.push('block');
        ...
        details.productState = {
          status: productStateRecord.status,
          productId: product.id
        };
      }
    }
  ...
  console.log(
    '✅ safety-engine загружен (синхронизирован)'
  );
})();